...
class customMDP(mdp):
        def __init__(self, initialState):
                ...
                dict_encadeado = lambda: defaultdict(dict_encadeado)        # Cria dicionario de dicionario, com base no codigo do repositorio oficial do livro
                self.Prob = dict_encadeado()
                self.actions = {(1,1): {"left": [[(1,1), 0.9], [(1,2), 0.1]], 
                                "right": [[(2,1), 0.8], [(1,2), 0.1], [(1,1), 0.1]], 
                                "up": [[(1,2), 0.8], [(2,1), 0.1], [(1,1), 0.1]], 
                                "down": [[(1,1), 0.9], [(2,1), 0.1]]}, 
        ...
        def getTransitionProbabilities(self, position, direction):      # Para as probabilidades aprendidas guardadas em P
            return [(prob, res) for (res, prob) in self.Prob[(position, direction)].items()]